...
# ---------------------------------------------------------------------------
# Payload builders for deterministic (httpx) tests
# ---------------------------------------------------------------------------

import json as _json


def make_layer0_payload(model, tool_name, arguments, *, tools=None):
    """Build an OpenAI chat payload with a tool_call in message history.

    This triggers Layer 0 scanning (request history).  ``arguments`` can be:
    - a string  → treated as {"command": string} for exec-like tools
    - a dict    → serialised as-is
    """
    if isinstance(arguments, str):
        arguments = {"command": arguments}
    args_json = _json.dumps(arguments) if isinstance(arguments, dict) else arguments
    ...
    return {
        ...
        "messages": [
            ...
            {"role": "tool", "tool_call_id": "call_evasion", "content": "done"},

...
func RunStartupWithPorts(defaultEndpoint string, defaultProxyPort, defaultAPIPort int) (StartupConfig, error) {
	...
	fmt.Println()
	fmt.Println("Crust - Agent Secure Gateway")
	fmt.Println(strings.Repeat("-", 40))
	fmt.Println()

	// Auto mode prompt (default: Yes)
	fmt.Print("Use auto mode? (resolve provider from model name, clients bring their own auth) [Y/n]: ")
	modeAnswer, _ := reader.ReadString('\n')
	modeAnswer = strings.TrimSpace(strings.ToLower(modeAnswer))

	if modeAnswer == "" || modeAnswer == "y" || modeAnswer == "yes" {
		// Auto mode
		config.AutoMode = true
		fmt.Println()
		fmt.Println("  Auto mode enabled. Providers will be resolved from model names.")
		fmt.Println("  Clients must provide their own auth headers.")
		fmt.Println()
	} else {
		// Manual mode — ask for endpoint and API key
